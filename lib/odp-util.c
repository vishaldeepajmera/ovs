    ...
    if (is_ip_any(flow) && !(flow->nw_frag & FLOW_NW_FRAG_LATER)) {
        if (flow->nw_proto == IPPROTO_TCP) {
            ...
            if (is_nd(flow, NULL)
                ...
                if(ntohl(data->igmp_group_ip4) != 0 || ntohl(data->tcp_flags) != 0) {
                    nd_ext_key = nl_msg_put_unspec_uninit(buf, OVS_KEY_ATTR_ND_EXTENSIONS,
                                                          sizeof *nd_ext_key);
    ...
    if (src_flow->nw_proto == IPPROTO_TCP
        ...
        if (present_attrs & (UINT64_C(1) << OVS_KEY_ATTR_ICMPV6)) {
            ...
            if (is_nd(src_flow, NULL)) {
                ...
                if (present_attrs & (UINT64_C(1) << OVS_KEY_ATTR_ND_EXTENSIONS)) {
                    const struct ovs_key_nd_extensions *nd_ext_key;
                    if (!is_mask) {
                        expected_attrs |= UINT64_C(1) << OVS_KEY_ATTR_ND_EXTENSIONS;
                    }

                    nd_ext_key = nl_attr_get(attrs[OVS_KEY_ATTR_ND_EXTENSIONS]);
...
static void
get_nd_extensions_key(const struct flow *flow, struct ovs_key_nd_extensions *nd_ext)
{
...
static void
put_nd_extensions_key(const struct ovs_key_nd_extensions *nd_ext, struct flow *flow)
{
...
static enum slow_path_reason
commit_set_nd_extensions_action(const struct flow *flow, struct flow *base_flow,
                                struct ofpbuf *odp_actions,
                                struct flow_wildcards *wc, bool use_masked)
{
    ...
    if (commit(OVS_KEY_ATTR_ND_EXTENSIONS, use_masked, &key, &base, &mask, sizeof key,
               odp_actions)) {
                     ...
                     bool use_masked)
{
    ...
    case ETH_TYPE_IPV6:
        ...
        if(ntohs(base->nw_proto == IPPROTO_ICMPV6)) {
            /* commit extended attrs first to make sure correct options are added. */
            reason = commit_set_nd_extensions_action(flow, base, odp_actions, wc, use_masked);
            reason |= commit_set_nd_action(flow, base, odp_actions, wc, use_masked);
            return reason;
        }
        break;
